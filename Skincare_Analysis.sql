...
SELECT c.customer_id, c.customer_name, o.order_id, o.final_amount
FROM customers c
JOIN orders o ON c.customer_id = o.customer_id
...
SELECT 
    DATE_FORMAT(STR_TO_DATE(order_date, '%d-%m-%Y'), '%m-%Y') AS month_year,
...
FROM products
WHERE concern LIKE 'Exfolation' AND (skin_type LIKE '%Oily%' OR skin_type LIKE '%Dry%')
GROUP BY skin_type
ORDER BY product_count DESC;
